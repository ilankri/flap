...
let int16_literal i = T.Literal (Util.Int16.of_int i)

let arg_reg_count = List.length Arch.Mips.argument_passing_registers

let sizeof word_count = int16_literal (word_count * Arch.Mips.word_size)

let load_immediate r i =
  try [T.Li (r, T.Literal (Util.Int16.of_int32 i))] with
  | Util.Int16.LiteralExceeds16bits _ ->
      [ T.Lui (r, T.Literal (Util.Int16.hi i))
      ; T.Xori (r, r, T.Literal (Util.Int16.low i)) ]

let increment_sp word_count =
  [T.Addiu (Arch.Mips.sp, Arch.Mips.sp, sizeof word_count)]

let decrement_sp word_count =
  [T.Addiu (Arch.Mips.sp, Arch.Mips.sp, sizeof (-word_count))]

let sp_offset_address offset =
  T.RegisterOffsetAddress (Arch.Mips.sp, sizeof offset)
...
let register (S.RId s) = Arch.Mips.register_of_string s
...
let tmp1 = Arch.Mips.tmp1
let tmp2 = Arch.Mips.tmp2
...
let translate (p : S.t) (_env : environment) : T.t * environment =
  ...
  and instruction stacksize env = function
    ...
    | S.TailCall (_, _) -> Util.ExtStd.failwith_todo __LOC__

    | S.Ret _ ->
        (* Pop the stack frame and then jump to the return address.  *)
        free_stack_frame stacksize @ [T.Jr Arch.Mips.ra]
    ...
    | S.Switch (_, _, _) -> Util.ExtStd.failwith_todo __LOC__

    | S.Comment s -> [T.Comment s]

    | S.Exit ->
        load_immediate (Arch.Mips.a 0) Int32.zero @ [T.J (T.Label "exit")]
  ...
  let main =
    ...
    let instrs =
      List.fold_left extract_instrs [Retrolix.FromFopix.labelled S.Exit]
        (List.rev p)
    in
