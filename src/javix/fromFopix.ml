...
module Env : sig
  ...
  let fresh_function_label =
    let fresh_suffix = Util.Gensym.make "_body_" in
...
let new_label =
  let fresh_suffix = Util.Gensym.make "_" in
...
let rec translate_expression (expr : S.expression) (env : environment) :
  ...
  | _ -> Util.ExtStd.failwith_todo __LOC__
...
and pass_fun_args args env =
  ...
  Util.ExtStd.List.flat_map (fun arg -> translate_expression arg env) args
...
let collect_function_info prog env =
  let collect_function_info env = function
    ...
    | S.DefineValue _ -> env
    | S.ExternalFunction _ -> Util.ExtStd.failwith_todo __LOC__
  in
