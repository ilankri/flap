...
module Mips = struct
  ...
  let all_registers = Util.ExtStd.List.(
    flatten [
      map t (range 0 9);
      map v [0];
      map a (range 0 3);
  ...
  let argument_passing_registers = Util.ExtStd.List.(
    map a (range 0 3)
  )

  let callee_saved_registers = Util.ExtStd.List.(
    Ra :: map s (range 0 7)
  )

  let caller_saved_registers = Util.ExtStd.List.(
