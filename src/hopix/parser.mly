...
let map_pos x y = Util.Position.map (fun _ -> x) y
...
let seq_expr es =
  ...
  let e, acc =
    ...
    | h :: t -> (h, t)
  in
  Util.Position.value (List.fold_left wrap e acc)
...
let flatten_patterns extract_patterns ps =
  ...
  let patterns p acc =
    match extract_patterns (Util.Position.value p) with
    | Some ps -> ps :: acc
    | None -> [p] :: acc
  in
...
%inline located(X):
  | x = X { Util.Position.with_poss $startpos $endpos x }
