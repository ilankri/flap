...
let error positions msg =
  Util.Error.errorN "execution" positions msg
...
type 'e gvalue =
  ...
  | VPrimitive of
      string * ('e gvalue Common.Memory.t -> 'e gvalue list -> 'e gvalue)
  | VFun          of pattern Util.Position.located list *
                     expression Util.Position.located * 'e
...
let print_value m v =
  ...
  let rec print_value d v =
    if d >= max_depth then "..." else
      ...
      | VUnit ->
          "()"
      | VAddress a ->
          print_array_value d (Common.Memory.dereference m a)
      | VTaggedValues (KId k, []) ->
          k
  ...
  and print_array_value d block =
    let r = Common.Memory.read block in
    let n = Int32.to_int (Common.Memory.size block) in
    "[ " ^ String.concat ", " (
      List.(map (fun i ->
        print_value (d + 1) (r (Int32.of_int i))
      ) (Util.ExtStd.List.range 0 (n - 1))
      )) ^ " ]"
  in
  print_value 0 v
...
module Environment : sig
  ...
  (** [update pos x env v] modifies the binding of [x] in [env] so
      that [x ↦ v] ∈ [env]. *)
  val update  : Util.Position.t -> identifier -> t -> t gvalue -> unit

  (** [lookup pos x env] returns [v] such that [x ↦ v] ∈ env. *)
  val lookup  : Util.Position.t -> identifier -> t -> t gvalue
  ...
  exception UnboundIdentifier of identifier * Util.Position.t
  ...
  val print   : t gvalue Common.Memory.t -> t -> string
end = struct
...
type observable = {
  new_memory      : value Common.Memory.t;
...
let initial_runtime () = {
  memory = Common.Memory.create (640 * 1024 (* should be enough. -- B.Gates *));
  environment = primitives;
}

(** Exception raised when a value cannot be captured by a certain
    pattern.  *)
exception Pattern_mismatch of Util.Position.t
...
let rec evaluate runtime ast =
  ...
  with Environment.UnboundIdentifier (Id x, pos) ->
    Util.Error.error "interpretation" pos (Printf.sprintf "`%s' is unbound." x)
...
and definition runtime d =
  match Util.Position.value d with
  | DefineValue (x, e) ->
...
and define_rec environment fs =
  ...
  let eval_and_rebind (id, fd) =
    let id, pos = Util.Position.destruct id in
    let v = Util.Position.located (function_definition environment) fd in
...
and expression' environment memory e =
  Util.Position.(expression (position e) environment memory (value e))
...
and expression position environment memory = function
  | Literal l -> (Util.Position.located literal l, memory)

  | Variable id ->
      Util.Position.(
        Environment.lookup (position id) (value id) environment, memory
      )
  ...
  | Tagged (c, _, es) ->
      let vs, memory = expressions environment memory es in
      (VTaggedValues (Util.Position.value c, vs), memory)
  ...
  | Case (e, branches) ->
      ...
      let rec aux = function
        ...
        | branch :: branches ->
            let Branch (p, e) = Util.Position.value branch in
  ...
  | Ref e ->
      let v, memory = expression' environment memory e in
      let location =
        try Common.Memory.allocate memory Int32.one v with
        | Common.Memory.OutOfMemory -> error [position] "Out of memory." in
  ...
  | Read e ->
      ...
      | Some addr ->
          (Common.Memory.(read (dereference memory addr) Int32.zero), memory)
      | None -> assert false    (* By typing.  *)
      end
  ...
  | Write (e1, e2) ->
      ...
      | Some addr ->
          ...
          Common.Memory.(write (dereference memory addr) Int32.zero v2);
          (VUnit, memory)
      | None -> assert false    (* By typing.  *)
      end
...
and pattern' environment v p =
  Util.Position.(pattern (position p) environment v (value p))
...
and pattern position environment v = function
  ...
  | PVariable id -> Environment.bind environment (Util.Position.value id) v

  | PTaggedValue (c, ps) ->
      begin match v with
      | VTaggedValues (c', vs) ->
          if Util.Position.value c = c' then patterns environment vs ps else
  ...
  | PLiteral l ->
      if Util.Position.located literal l = v then environment else
        raise (Pattern_mismatch position)
...
and bind_identifier environment x v =
  Environment.bind environment (Util.Position.value x) v
