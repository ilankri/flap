...
let located f x = f (Util.Position.position x) (Util.Position.value x)

let rec vars_of_pattern' p = vars_of_pattern (Util.Position.value p)

and vars_of_pattern = function
  | PTypeAnnotation (p, _) -> vars_of_pattern' p
  | PVariable x -> [Util.Position.value x]
...
let check_linear_pattern pos p =
  try
    let id = Util.ExtStd.List.find_duplicate (vars_of_pattern p) in
...
let check_program_is_fully_annotated ast =
  ...
  let rec program p =
    List.iter (fun item -> definition (Util.Position.value item)) p

  and definition = function
    | DefineValue (_, e) -> located expression e
  ...
  and function_definition need_return_type pos = function
    | FunctionDefinition (_, ps, e) ->
        ...
        if need_return_type then
          match Util.Position.value e with
  ...
  and expression pos = function
    ...
    | Case (e, branchlist) ->
        located expression e;
        List.iter (fun br -> branch (Util.Position.value br)) branchlist;
    | TypeAnnotation (e, _) | Ref e | Read e ->
        located expression e
    | Literal _ | Variable _ -> ()

  and pattern pos = function
    | PTypeAnnotation ({ Util.Position.value = (PWildcard | PVariable _) }, _) |
      PLiteral _ -> ()
...
let typecheck tenv ast : typing_environment =
  ...
  let rec program p =
    List.fold_left (fun env x -> definition env (Util.Position.value x)) tenv p
  ...
  and rec_definition tenv recdefs =
    ...
    let fids = List.map Util.Position.value fids in
    let tenv =
      try
  ...
  and definition tenv = function
    | DefineValue (x, e) ->
        bind_value
          (Util.Position.value x) (type_scheme_of_expression' tenv e) tenv

    | DefineRecFuns recdefs -> rec_definition tenv recdefs

    | DefineType (t, ts, tdef) ->
        let tyListNoPosition = List.map Util.Position.value ts in
        bind_type_definition (Util.Position.value t) tyListNoPosition tdef tenv

    | DeclareExtern (x, ty) ->
        let x = Util.Position.value x in
  ...
  (* [extract_function_type_scheme tenv pos fdef] constructs a type
     ...
     definition actually has the type scheme written by the
     programmer. *)
  and extract_function_type_scheme tenv pos (FunctionDefinition (ts, ps, e)) =
    ...
    let tenv =
      let tvs = List.map Util.Position.value ts in
  ...
  (* [check_function_definition tenv pos fdef] checks that the function
     definition [fdef] is well-typed with respect to the type
     annotations written by the programmer. We assume that [tenv]
     already contains the type scheme of the function [f] defined by
     [fdef] as well as all the functions which are mutually recursively
     defined with [f]. *)
  and check_function_definition tenv pos (FunctionDefinition (tvs, ps, e)) =
    let tenv =
      bind_type_variables pos tenv (List.map Util.Position.value tvs)
    in
    let tenv, ts = patterns tenv ps in
    let Scheme (_, t) = type_scheme_of_expression' tenv e in
    mk_type_scheme (ATyArrow (ts, t))

  (* [check_expected_type pos xty ity] verifies that the expected type
     [xty] is syntactically equal to the inferred type [ity] and raises
     an error otherwise. *)
  and check_expected_type pos xty ity =
    if xty <> ity then raise_type_error (TypeMismatch (pos, xty, ity))

  (* [check_expression_monotype tenv xty e] checks if [e] has the
     monotype [xty] under the context [tenv]. *)
  and check_expression_monotype tenv xty e =
    let pos = Util.Position.position e in
  ...
  (* [type_scheme_of_expression tenv pos e] computes a type scheme for
     [e] if it exists. Besides, this type scheme must be a monotype if
     no type instanciation is provided in the enclosing expression. *)
  and type_scheme_of_expression tenv pos = function
    ...
    | Define (x, e, e') ->
        ...
        let tenv' = bind_value (Util.Position.value x) sigma tenv in
        let sigma' = type_scheme_of_expression' tenv' e' in
        sigma'
    ...
    | Apply (a, types, args) ->
        let t =
          let pos = Util.Position.position a in
    ...
    | If (eelist, expr) ->
        ...
        let check_branch (cond, expr) =
          ...
          check_expected_type (Util.Position.position expr) else_ty t
        in
    ...
    | Write (e, e') ->
        ...
        check_expected_type (Util.Position.position e') tau tyToWrite;
        monotype hunit
    ...
    | Literal l -> monotype (type_of_literal (Util.Position.value l))
  ...
  (* [apply pos tenv s types args] computes the instanciation of the
      type scheme [s] with [types] and checks that the resulting type is
      an arrow. The input types of this arrow must correspond to the
      types of the expressions [args]. *)
        ...
        let check_args () =
          try
            List.iter2 (fun xty arg ->
              let pos = Util.Position.position arg in
  ...
  (* [pattern tenv pos p] computes a new environment completed with the
      variables introduced by the pattern [p] as well as the type of
      this pattern. *)
  ...
  and pattern' tenv pos = function
    (*
       ———————————————————————
       Γ ⊢ x : τ ⇒ Γ'(x : τ), τ *)
    | PTypeAnnotation ({ Util.Position.value = PVariable x }, ty) ->
        let aty = internalize_ty tenv ty in
        let tenv' = bind_value (Util.Position.value x) (monotype aty) tenv in
        tenv', aty

    (*
       ————————————————
       Γ ⊢ _ : τ ⇒ Γ, τ *)
    | PTypeAnnotation ({ Util.Position.value = PWildcard }, ty) ->
    ...
    | PTaggedValue (k, ps) ->
        ...
        in
        let t = check_ptagged aty_scheme ts in
    ...
    *)
        ...
        let checkEachPEqual (prevEnv, prevAty) p =
          (
            ...
            let prevEnv = match prevEnv with
              | Some x ->
                  if (x<>gammai) then
                    raise_type_error (POrError (Util.Position.position p));
                  Some gammai
              | None -> Some gammai in
            match prevAty with
            | Some x ->
                check_pattern_type (Util.Position.position p) pAty x;
                (prevEnv, Some pAty)
    ...
    | PLiteral l -> tenv, type_of_literal (Util.Position.value l)

  (* [branches tenv sty bs] checks that the patterns of the branches
      [bs] have type [sty] and that the bodies of these branches all
      have the same type. *)
  and branches tenv sty bs =
    let oty =
      List.fold_left (fun oty b ->
        branch tenv sty oty (Util.Position.value b)
      ) None bs
    in
    match oty with
  ...
  and branch tenv sty oty = function
    | Branch (p, e) ->
        let envP, tyP = located (pattern tenv) p in
        let Scheme (_, atyE) = type_scheme_of_expression' envP e in
        check_pattern_type (Util.Position.position p) tyP sty;
        match oty with
        | Some x ->
            check_expected_type (Util.Position.position e) x atyE;
            Some atyE
        | None -> Some atyE
