...
and aty_of_ty' x = aty_of_ty (Util.Position.value x)
...
let type_error = Util.Error.error "typechecking"

type error =
  | UnboundTyCons of Util.Position.t * type_constructor
  | UnboundTyVar of Util.Position.t * type_variable
  | UnboundId of Util.Position.t * identifier
  | UnboundDataCons of Util.Position.t * constructor
  | WrongArityTyCons of Util.Position.t * type_constructor * int * int
  | WrongArityDataCons of Util.Position.t * constructor * int * int
  | WrongArityFunction of Util.Position.t * int * int
  | AlreadyBoundDataCons of Util.Position.t * constructor
  | AlreadyBoundTyVar of Util.Position.t * type_variable
  | InvalidApplication of Util.Position.t
  | MissingTypeAnnotation of Util.Position.t
  | TypeMismatch of Util.Position.t * aty * aty
  | TooPolymorphicType of Util.Position.t
  | POrError of Util.Position.t
  | InvalidTypeInstantiation of Util.Position.t * int * int
  | NonLinearPattern of Util.Position.t * identifier
...
let unify_types ty1 ty2 =
  ...
  if (substitute phi ty1 <> substitute phi ty2) then (
    Util.Error.global_error "internal" (
...
let check_ty_cons_data_cons pos data_constructors =
  try
    let dc = Util.ExtStd.List.find_duplicate data_constructors in
...
let internalize_ty env ty =
  let pos = Util.Position.position ty in
  let ty = Util.Position.value ty in
...
let bind_type_definition x ts tdef env =
  ...
  let data_constructors = match tdef with
    | Abstract -> []
    | DefineSumType ds -> List.map (fun (k, _) -> Util.Position.value k) ds
  in
  let pre_env =
    let env = bind_type_variables Util.Position.dummy env ts in
  ...
  let constructor_definition (k, tys) =
    ...
    let aty =
      let ts = List.map (fun v -> Util.Position.unknown_pos (TyVar v)) ts in
      internalize_ty pre_env (Util.Position.unknown_pos (TyCon (x, ts)))
    in
    let scheme =
      mk_type_scheme (atys --> aty)
    in
    (Util.Position.value k, scheme)
  in
